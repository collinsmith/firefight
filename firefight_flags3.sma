...
#define MODEL_FLAGS "models/ff/firefight_flags.mdl"
#define FILE_DOMCFG "/bf2flags/%s.cfg"
...
#define SOUND_MATCHWIN_T	"sound/firefight/t_win.mp3"
#define SOUND_ROUNDWIN_T	"firefight/round_win.wav"
#define SOUND_MATCHWIN_CT	"sound/firefight/ct_win.mp3"
#define SOUND_ROUNDWIN_CT	"firefight/round_win.wav"
#define SOUND_MATCHLOSE_T	"sound/firefight/t_loss.mp3"
#define SOUND_ROUNDLOSE_T	"firefight/round_lose.wav"
#define SOUND_MATCHLOSE_CT	"sound/firefight/ct_loss.mp3"
#define SOUND_ROUNDLOSE_CT	"firefight/round_lose.wav"
#define SOUND_LOWFORCE		"firefight/team_losing.wav"

#define HUD_LINE_MAXSIZE 32
#define HUD_FLAGS_STRING_MAXSIZE HUD_LINE_MAXSIZE * MAX_CSFLAGS

#define MSG_HEADER "[Flags]"

#define TEAM_DESC_CT "USMC"
#define TEAM_DESC_T "MEC"

#define IFW_FLAGTAKEN "csf_flag_taken"
#define IFW_FLAGTAKEN2 "csf_flag_taken_assist"
...
new g_Teams[2][] = {"Middle Eastern Coalition", "U.S. Marine Corps"}

/** Default notify sounds */
new g_NotifySound[2][] = {"firefight/area_secure_t.wav", "firefight/area_secure_ct.wav"}
new g_NotifySoundLost[2][] = {"firefight/obj_lost_t2.wav", "firefight/obj_lost_ct2.wav"}
...
{
	"MEC (Ts) | Wins: %d/%d^nRespawns Remaining: %d",
	"USMC (CTs) | Wins: %d/%d^nRespawns Remaining: %d"
}
...
public plugin_init() 
{
	register_plugin("FireFight Flags", VERSION, "commonbullet & CSDM Team & Tirant")
	
	register_event("SendAudio", "eventEndRound", "a", "2=%!MRAD_terwin", "2=%!MRAD_ctwin", "2=%!MRAD_rounddraw")
	register_event("SendAudio", "eventEndRound_T", "a", "2=%!MRAD_terwin")
	register_event("SendAudio", "eventEndRound_CT", "a", "2=%!MRAD_ctwin")
	...
	g_cvarMaxWins = register_cvar("amx_csflags_max_wins", "4")
	g_cvarMaxWinsCache = get_pcvar_num(g_cvarMaxWins);
	//g_cvarMaxPoints = register_cvar("amx_csflags_max_points", "100")
	g_cvarDomEnabled = register_cvar("amx_csflags_enabled", "1")
	g_cvarMapEndTime = register_cvar("amx_csflags_mapendtime", "1.0")
	...
	g_cvarWinFrags = register_cvar("amx_csflags_winfrags", "10")
	g_cvarCaptureFrags = register_cvar("amx_csflags_capturefrags", "2")
	g_cvarCaptureDelay = register_cvar("amx_csflags_capturedelay", "7")
	g_cvarMatchRespawn = register_cvar("amx_csflags_respawn", "2")
	g_cvarFreeze = register_cvar("amx_csflags_freeze", "0")
	g_cvarVoteStartCommand = register_cvar("amx_csflags_vote_startcmd", "")
	//g_cvarVoteStartCommand = register_cvar("amx_csflags_vote_startcmd", "gal_startvote")
...
public plugin_precache() 
{
	...
	precache_sound(g_NotifySoundLost[0])
	precache_sound(g_NotifySoundLost[1])
	
	precache_generic(SOUND_MATCHWIN_T);
	precache_sound(SOUND_ROUNDWIN_T);
	precache_generic(SOUND_MATCHWIN_CT);
	precache_sound(SOUND_ROUNDWIN_CT);
	precache_generic(SOUND_MATCHLOSE_T);
	precache_sound(SOUND_ROUNDLOSE_T);
	precache_generic(SOUND_MATCHLOSE_CT);
	precache_sound(SOUND_ROUNDLOSE_CT);
	precache_sound(SOUND_LOWFORCE);
}
...
public exec_vote_cmd()
{
	...
	callfunc_begin("vote_startDirector","galileo.amxx")
	callfunc_push_int ( true );
	callfunc_end()
}
...
show_win_hud(team)
{
	...
	if(get_playersnum()) {		
		...
		for(new i = 0; i < playersnum; i++) {			
			for(new p = 0; p < 5; p++) {
				currentscore = g_iPlayerScore[players[i]]
				if(currentscore == 0)
					continue
	...
	len += format(message[len], 255 - len, "%s^n", MSG_HEADER)
	...
	*/
	if(g_MatchLastWin)
	{
		...
		len += format(message[len], 255 - len, "%s have won the match!", g_Teams[team])
		//start_vote_cmd();
	}
	else
		len += format(message[len], 255 - len, "%s have won this flag round!", g_Teams[clamp(team, 0, 1)])
	
	/* Adding the best scores */
	len += format(message[len], 255 - len, "^n^nBest scores in this match:^n")
	
	for(new i = 0; i < count; i++) {
		get_user_name(bestplayer[i], name, 31)
		//len += format(message[len], 255 - len, "^n%s - %d", name, floatround(g_PlayerScore[bestplayer[i]]))		
		len += format(message[len], 255 - len, "^n%s - %d Flag Captures", name, g_iPlayerScore[bestplayer[i]])		
	}
		...
		for(new i = 0; i < playersnum; i++) {
			...
			if((t = get_user_team(players[i]))) {
				...
				if((t - 1) == team) 
				{
					if (team == ID_TEAM_T)
					{
						if(g_MatchLastWin) client_cmd(players[i], "mp3 play %s", SOUND_MATCHWIN_T)
						else client_cmd(players[i], "spk %s", SOUND_ROUNDWIN_T)
					}
					else
					{
						if(g_MatchLastWin) client_cmd(players[i], "mp3 play %s", SOUND_MATCHWIN_CT)
						else client_cmd(players[i], "spk %s", SOUND_ROUNDWIN_CT)
					}
				}
				else if((t - 1) == (team ^ 1)) 
				{
					if (team == ID_TEAM_T)
					{
						if(g_MatchLastWin) client_cmd(players[i], "mp3 play %s", SOUND_MATCHLOSE_T)
						else client_cmd(players[i], "spk %s", SOUND_ROUNDLOSE_T)
					}
					else
					{
						if(g_MatchLastWin) client_cmd(players[i], "mp3 play %s", SOUND_MATCHLOSE_CT)
						else client_cmd(players[i], "spk %s", SOUND_ROUNDLOSE_CT)
					}
				}
			}
...
public restart_counter() 
{
	...
	g_hasSaid[_TEAM_CT] = false;
	g_hasSaid[_TEAM_T] = false;
	return PLUGIN_CONTINUE
}
...
public flag_captured(const info[])
{
	...
	{
		if( !is_user_alive( player ) || team != get_user_team(player) || player == toucher) continue;
		if(g_ifw_FlagTaken2) 
		{
			new retval
	...
	for(new i = 1; i <= g_MaxPlayers; i++) {
		...
		if(get_pcvar_num(g_cvarUseRadio) && (team == 1 || team == 2)) {
			if(team == toucherteam) {
				...
				client_print_color(i, DontChange, "%s ^1[^4%d^1]^3%s^1(^3%d^1) ^1captured the ^4[%s] ^1flag for the ^3%s", MODNAME, ff_get_user_rank(toucher), name, g_iPlayerScore[toucher],  g_PlaceNames[flagid], (toucherteam == 2 ? TEAM_DESC_CT : TEAM_DESC_T));
			}
			
			else  
			{
				if((team - 1) == ((toucherteam - 1) ^ 1))
					client_cmd(i, "spk %s", g_NotifySoundLost[team - 1])
				client_print_color(i, DontChange, "%s ^1[^4%d^1]^1%s^1(^3%d^1) ^1captured the ^4[%s] ^1flag for the %s", MODNAME, ff_get_user_rank(toucher), name, g_iPlayerScore[toucher], g_PlaceNames[flagid], (toucherteam == 2 ? TEAM_DESC_CT : TEAM_DESC_T));
			}			
...
move_to_spawn()
{
	...
	for(new i = 0; i < playersnum; i++) {
		...
		else if (players[i] && !is_user_alive(players[i]))
		{
			csdm_respawn(players[i])
		}
...
{	
	...
	{
		formatex(headerT, 63, g_ScoreHeaderFormat[ID_TEAM_T], g_TeamWins[ID_TEAM_T], g_cvarMaxWinsCache, t)
		formatex(headerCT, 63, g_ScoreHeaderFormat[ID_TEAM_CT], g_TeamWins[ID_TEAM_CT], g_cvarMaxWinsCache, ct)
		
		new players[32]
		new playersnum
		get_players(players, playersnum)
		
		if (t == 25 && !g_hasSaid[_TEAM_T])
		{
			g_hasSaid[_TEAM_T] = true;
			for(new i = 0; i < playersnum; i++) {
				
				if(!is_user_connected(players[i]))
					continue
					
				if(get_user_team(players[i])-1 == ID_TEAM_T) 
				{
					client_cmd(players[i], "spk %s", SOUND_LOWFORCE)
				}
			}
		}
		
		if (ct == 25 && !g_hasSaid[_TEAM_CT])
		{
			g_hasSaid[_TEAM_CT] = true;
			for(new i = 0; i < playersnum; i++) {
				
				if(!is_user_connected(players[i]))
					continue
					
				if(get_user_team(players[i])-1 == ID_TEAM_CT) 
				{
					client_cmd(players[i], "spk %s", SOUND_LOWFORCE)
				}
			}
	...
	for(ind = 1; ind <= g_MaxPlayers; ind++) {
		if(is_user_connected(ind)) {
			if(is_user_alive(ind)) {	
				set_hudmessage(255, 0, 0, 0.3, 0.0, 0, 6.0, INTERVAL_SCORE_UPDATE, 0.1, 0.2, 2)
				show_hudmessage(ind, scoreT)
				set_hudmessage(ind, 0, 255, 0.6, 0.0, 0, 6.0, INTERVAL_SCORE_UPDATE, 0.1, 0.2, 3)
				show_hudmessage(ind, scoreCT)
			}
			else {
				set_hudmessage(255, 0, 0, 0.3, 0.17, 0, 6.0, INTERVAL_SCORE_UPDATE, 0.1, 0.2, 2)
				show_hudmessage(ind, scoreT)
				set_hudmessage(ind, 0, 255, 0.6, 0.17, 0, 6.0, INTERVAL_SCORE_UPDATE, 0.1, 0.2, 3)
				show_hudmessage(ind, scoreCT)
			}
		}			
	}
...
public force_end() 
{
	...
	if (get_playersnum() && mplimit != 0.0) {
		...
		client_print_color(0, DontChange, "%s ^4Flags match is over, waiting for the next map", MODNAME);
	...
	if( (g_CSFlagsState & GAMESTATE_STARTED) &&
		...
		if(!get_pcvar_num(g_cvarTieMatch)) {			
			...
			if(!map_should_end()) {
				...
				client_print_color(0, DontChange, "%s ^4Time is over. Waiting for a winner...", MODNAME);
...
public eventEndRound_T()
	team_win(ID_TEAM_T) 

public eventEndRound_CT()
	team_win(ID_TEAM_CT)
